    ...
    try {
        for (var i = 0, len = newpath.length, p, v; i < len; i++) {
            ...
            if (i != len) {
                ...
                v = data.getIn(p);
                if (!v) {
                    data = typeof v == 'undefined' ? data.mergeIn(p.slice(0, p.length - 1), _immutable2['default'].fromJS(_defineProperty({}, p[p.length - 1], {}))) : data.setIn(p, {});
                }
            }
        }
    } catch (ex) {
...
function Model(target) {
    ...
    var store = (0, _gfsReactReduxTwowayBinding.createReducer)(modelName, _immutable2['default'].fromJS(params.property || {}), params.method);
